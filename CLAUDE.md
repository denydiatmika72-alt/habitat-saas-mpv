...
- **Sidebar nav grouping** (`client/src/components/dashboard/sidebar.tsx`): tiap item `nav` boleh punya field opsional `group` (`"Perencanaan" | "Kerjasama" | "Operasional" | "Keuangan" | "Tiket & Pencairan"`) → dirender sebagai section collapsible (state `useState`, default terbuka, tidak dipersist) sesuai urutan `GROUP_ORDER`. Item TANPA `group` render ungrouped seperti biasa: "Dashboard" di atas, item `adminOnly` di bawah, item `hidden` (mis. "Vendor & Talent") tetap terfilter. Grouping murni rendering — filter `!item.hidden && (!item.adminOnly || isAdmin)` jalan DULU sebelum grouping; href/badge/Pro-gating tidak disentuh. **Grouping murni tematik, BUKAN proxy tier harga**: dalam satu group, item boleh beda status Pro-gating — mis. di "Perencanaan", "RAB Builder" gratis/Starter (tanpa badge) berdampingan dengan "Simulasi Harga Tiket" yang Pro (ada `badge: "Pro"`).
