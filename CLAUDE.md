...
**⚠️ KOREKSI 2026-07-24: audit 2026-07-20 ternyata BELUM tuntas untuk PO** — ia hanya menutup endpoint LIST;
endpoint by-`:id` PO (`getPOById`/`updatePO`/`deletePO`/`addPOItem`/`deletePOItem`/`generatePurchaseOrderPdf`)
masih tanpa cek pemilik (IDOR: user login mana pun bisa baca/ubah/hapus PO promotor lain bermodal UUID).
**Sudah DIFIX 2026-07-24** via helper `findOwnedPO` (satu query join event → 404/403) di
`purchaseOrder.controller.js` — lihat known-bugs [2026-07-24]. Catatan lama "mutasi PO by-id belum semua punya
guard, kandidat audit lanjutan" (known-bugs [2026-07-18]) dengan ini SUDAH DITINDAKLANJUTI & tertutup.
...
15. ✅ Event Summary Report (kirim via email saat event selesai) — **DEPLOYED & VERIFIED di production**
    (dikonfirmasi audit 2026-07-23: commit `16c9d75` adalah ancestor dari HEAD ter-deploy di VPS, smoke test
    endpoint 401-not-404 lolos; wording "PENDING DEPLOY" yang lama sudah STALE dan dikoreksi 2026-07-24).
    Laporan akhir 1 event (9 seksi: keuangan, sponsor+status
...
- **MobileNav (bottom-nav mobile) = HUB-ONLY sejak 2026-07-24** (keputusan founder, menyelaraskan dgn desktop): `mobileNavItems` di `sidebar.tsx` = Dashboard / Perencanaan / Kerjasama / Tiket / Keuangan (5 hub, ikon khas mengikuti kartu Akses Cepat KPI: Handshake/Ticket/Wallet — bukan BarChart2 seragam ala desktop, tak terbedakan di bottom-nav). Quick-link lama "Sponsor" (`/dashboard/sponsor`) & "Invoice" (`/dashboard/invoice`) DIHAPUS — keduanya turunan Dashboard Kerjasama yang sudah dicabut dari sidebar desktop sejak 2026-07-18; mobile tertinggal (catatan "kandidat penyelarasan" di known-bugs [2026-07-18] dgn ini tertutup). JANGAN tambahkan kembali link non-hub ke mobileNavItems.
- **Ikon lonceng notifikasi DIHAPUS 2026-07-24** (keputusan founder: hapus, bukan bangun) dari top-bar dashboard (`top-bar.tsx`) DAN sponsor-dashboard (`sponsor-dashboard/page.tsx`) — murni dekoratif (tanpa backend/onClick, titik hijau hardcoded), menyesatkan user seolah ada sistem notifikasi. TIDAK ada sistem notifikasi in-app di aplikasi ini. JANGAN kembalikan ikonnya tanpa membangun backend notifikasi sungguhan lebih dulu.
