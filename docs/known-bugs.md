...
- Catatan penting (temuan saat verifikasi — storefront publik ternyata setengah jadi dari sesi sebelumnya):
  ...
  - **Keterbatasan diketahui (BELUM ditangani, perlu keputusan):** webhook `handleTicketOrderWebhook` generate e-ticket QR hanya dari `order.items` (TicketOrderItem). Tiket YANG BERADA DI DALAM PAKET disimpan sebagai `BundleOrderItem` (tanpa TicketOrderItem), sehingga pembeli paket berisi tiket TIDAK dapat e-ticket QR. Fix butuh keputusan skema (buat TicketOrderItem untuk tiket paket TANPA menggandakan hitungan NIK/stok, atau ubah relasi Ticket). Belum diimplementasi karena di luar scope eksplisit task & berisiko ripple ke NIK counting.
...
- Verifikasi: `npx prisma db push` → "already in sync" (tabel bundle sudah ada di Supabase); Prisma client sudah ter-generate (punya `bundlePackage`); `node --check` lolos semua file server; `npx tsc --noEmit` client EXIT 0; `npm run build` client sukses (21 route). Verifikasi E2E (buat paket → approve → beli paket, stok tiket+merch turun; beli tiket+merch mixed → fee terpisah bukan bundling) pending setelah deploy Mandor di VPS (SSH key tidak ada di PC ini).
- Tag: #bundling #storefront #fee-platform #prisma #schema #checkout #mixed-order #approval #idr-format
