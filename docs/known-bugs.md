...
- Belum deploy (per instruksi — tunggu instruksi deploy eksplisit). CATATAN: kolom `category`/`platform` sudah ter-`db push` ke Supabase production (nullable, aman utk kode lama), tapi KODE belum di-deploy ke VPS.
- Tag: #pl-report #critical #income #ticket-revenue #payout-parity #other-income #category #platform #tiket-platform-lain #double-count-guard #backward-compat #pdfkit #schema-change #db-push
