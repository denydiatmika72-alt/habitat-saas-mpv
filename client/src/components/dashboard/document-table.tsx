...
export function DocumentTable() {
  ...
  const filteredEvents =
    filter === "Invoice"
      ? events.filter((e) => invoicesByEventId.has(String(e.id)))
      : events

  const colSpan = filter === "Invoice" ? 7 : 6
            ...
            {isLoading ? (
                  ...
                  {filter === "Invoice" ? (
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm text-slate-500">
                        Belum ada event dengan invoice terhubung.
                      </p>
                      <p className="text-xs text-slate-400">
                        Generate invoice dari halaman Invoice & pilih deal yang terhubung ke event.
                      </p>
              ...
              filteredEvents.map((event) => (
                <EventTableRow
                  ...
                  showInvoiceColumns={filter === "Invoice"}
...
function EventTableRow({
  ...
  return (
    <TableRow className="border-slate-200 transition-colors hover:bg-slate-50">
      ...
      {/* Kolom dinamis: Invoice atau Nilai */}
      {showInvoiceColumns ? (
        <>
          <TableCell>
            {invoiceInfo ? (
          ...
          <TableCell>
            {invoiceInfo ? (
              <span className="text-sm text-slate-700">{formatDate(invoiceInfo.invoiceDate)}</span>
            ) : (
              <span className="text-xs text-slate-400">—</span>
            )}
          </TableCell>
          <TableCell>
            {invoiceInfo ? (
              <Badge
                variant="outline"
                className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${invoiceStatusBadge(invoiceInfo.invoiceStatus)}`}
              >
      ...
      <TableCell>
        <div className="flex flex-col gap-1">
          {isLoadingBudget ? (
            ...
            <Badge
              variant="outline"
              className="rounded-full px-2.5 py-0.5 text-xs font-medium border-emerald-200 bg-emerald-50 text-emerald-700"
            >
              Ada RAB
            </Badge>
          ) : (
            <Badge
              variant="outline"
              className="rounded-full px-2.5 py-0.5 text-xs font-medium border-amber-200 bg-amber-50 text-amber-700"
            >
              Belum Ada RAB
            </Badge>
          )}
          {invoiceInfo ? (
            ...
            <Badge
              variant="outline"
              className="rounded-full px-2.5 py-0.5 text-xs font-medium border-slate-200 bg-slate-50 text-slate-500"
            >
              Belum Ada Invoice
            </Badge>
          )}
